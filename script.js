...
`;
  ...
  // append with toggle BEFORE the trash
  row.append(day, time, toggleWrap, remove);
...
async function saveSchedule() {
  ...
  const desired = Array.from(document.querySelectorAll('#scheduleRows .schedule-row')).map(r => {
    ...
    return time ? { day_of_week: day, time_local: time, buoi_phu: bp, timezone: tz } : null;
  }).filter(Boolean);
